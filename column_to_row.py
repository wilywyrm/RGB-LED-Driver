...
for char in font_data_column:
    char_rows = [0]*8
    for col in range(0,5):              # the column of the char
        for colbit in range(0,8):       # the bitmask for column
            if((1 << (7-colbit)) & char[col]):
                char_rows[colbit] |= (1 << (7-col))
                #print "1 at pos ", colbit
    #print char_rows
    font_data_row.append(char_rows)

f = open('font_data_row.txt', "w");



for char in font_data_row:
    f.write("{")
    for row in char:
        f.write("0x")
        f.write("%02x, " % row)
